...
app.get("/api/trending", async (req, res) => {
  try {
    ...
    const validResults = data.results.filter((m: any) => m.media_type === "movie" || m.media_type === "tv");
    
    const enrichedResults = await Promise.all(validResults.slice(0, 20).map(async (m: any) => {
...
if (!code.includes('/api/trending')) {
  code = code.replace(/app\.get\("\/api\/search",/g, trendingRoute + '\napp.get("/api/search",');
  fs.writeFileSync('server.ts', code, 'utf-8');
}
console.log("Added trending route");
