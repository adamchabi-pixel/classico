...
pattern = re.compile(r'const tryFetch = async \(\) => \{[\s\S]*?\} catch \(err\) \{[\s\S]*?\}', re.MULTILINE)
...
new_tryFetch = """const tryFetch = async () => {
        try {
          ...
          if (attempts < maxAttempts - 1) {
            ...
            setJellyfinConfig({ configured: false });
            setConnectionError("Impossible de se connecter au serveur distant.");
          }
        }"""
