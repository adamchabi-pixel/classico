...
file = file.replace(/const \[showWelcomeModal, setShowWelcomeModal\] = useState\(false\);/, '');

file = file.replace(/useEffect\(\(\) => \{\s*if \(\!localStorage\.getItem\("classico_welcome_shown"\)\) \{\s*const timer = setTimeout\(\(\) => \{\s*setShowWelcomeModal\(true\);\s*\}, 100\);\s*return \(\) => clearTimeout\(timer\);\s*\}\s*\}, \[\]\);/, '');

const targetModal = `{/* WELCOME POPUP MODAL */}
      <AnimatePresence>
        {showWelcomeModal && (
          <motion.div
            initial={{ opacity: 0 }}
...
file = file.replace(targetModal, '');

fs.writeFileSync('src/App.tsx', file);
console.log("Removed Welcome Modal");
