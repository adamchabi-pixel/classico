          ...
          {searchQuery.trim() !== "" ? (
            ...
            >
              ...
              <div className="space-y-8 text-left">
                ...
                {/* Key Icon for Admin Unlock */}
                <div className="flex justify-end">
                  <button
                    onClick={() => {
                      if (!isAdminUnlocked) {
                        const pin = window.prompt("Code secret Admin :");
                        if (pin === "0698") {
                    ...
                    className="text-zinc-900 hover:text-zinc-600 transition-colors opacity-30 hover:opacity-100"
                    title="Unlock Admin"
                  >
                    🔑
                  </button>
                ...
                <div className="bg-zinc-900/50 border border-zinc-800 rounded-xl p-6 space-y-4">
                  <h3 className="text-lg font-display font-bold uppercase tracking-wider text-white flex items-center gap-2">
                    ...
                    Administration - Import en Masse
                  </h3>
                  <p className="text-zinc-400 text-sm">
                    Utilise l'API TMDb et le lecteur VidAPI pour ajouter automatiquement des films au catalogue.
                  </p>
                  
                  <div className="space-y-3">
                    <label className="text-sm font-medium text-zinc-300">Entrez les IMDb IDs ou TMDb IDs (Séparez-les par une virgule pour un ajout multiple)</label>
                    <textarea 
                      id="bulk-import-ids"
                      className="w-full bg-black border border-zinc-700 rounded-lg p-3 text-white placeholder-zinc-500 font-mono text-sm resize-y min-h-[100px]"
                      placeholder="Ex: tt33764258, tt29623480, 1184918"
                    ></textarea>
                  </div>

                  <button
                    onClick={async () => {
                      const inputEl = document.getElementById("bulk-import-ids") as HTMLTextAreaElement;
                      if (!inputEl || !inputEl.value.trim()) return alert("Veuillez entrer au moins un ID");
                      ...
                      try {
                        const res = await fetch("/api/admin/movies/add", {
                          method: "POST",
                          headers: { "Content-Type": "application/json" },
                          body: JSON.stringify({ ids: inputEl.value })
                        });
                        const data = await res.json();
                        if (data.success) {
                          setTestOdysseySuccess(`Succès ! ${data.count} film(s) ajouté(s) au catalogue.`);
                          inputEl.value = "";
                          loadJellyfinLibrary();
                          setTimeout(() => setTestOdysseySuccess(null), 5000);
                        } else {
                    ...
                    disabled={isTestingOdyssey}
                    className="bg-indigo-600 hover:bg-indigo-500 text-white px-6 py-2.5 rounded-lg font-medium transition-colors flex items-center gap-2 disabled:opacity-50"
                  >
                    {isTestingOdyssey ? (
                      <>
                      ...
                      "Lancer l'automatisation du catalogue"
                    )}
                  </button>
                  {testOdysseySuccess && (
